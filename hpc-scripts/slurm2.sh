...
#
# Hostname: auto-detected from `hostname -s` (which 01_os_base.sh
# already set). Override only if needed:
#   sudo NODE_HOSTNAME=cpu01 ./05_slurm.sh
# ============================================================
set -euo pipefail
...
# Pattern: <prefix>cpu01 / <prefix>gpu01 / <prefix>gpu02 / <prefix>gpu03
# Auto-detects the prefix from the local hostname so customer
# naming conventions (e.g., insiiukcpu01) flow through to slurm.conf.
...
echo "[*] Hostname prefix detected: '${HOSTNAME_PREFIX:-<none>}'"
...
if command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi >/dev/null 2>&1; then
  ...
  # Cross-check against canonical config (suffix-match handles prefixed names)
  case "$NODE_HOSTNAME" in
    *gpu01|*gpu02)
      [[ "$GPU_TYPE" == "h200_nvl" ]] || \
