...
# Refuse to run on anything but cpu01 — slurmdbd lives on the controller only
if [[ "$NODE_HOSTNAME" != "cpu01" ]]; then
  ...
  echo "    they reach slurmdbd over the wire via AccountingStorageHost=cpu01."
  exit 1
fi
